...
class Node{
    ...
    show(col){
        fill(col?? '#333')
        rectMode(CENTER)
	if(this.type == 'AND' || this.type == 'AND3')
	{
		rect(this.x, this.y, this.w, this.h,0,20,20,0) ;
	}else{
        ...
        for (let i = 0; i < this.outputPos.length; i++) {
            rect(this.x+this.outputPos[i].x, this.y+this.outputPos[i].y, 12, 10,10);
        ...
        for (let i = 0; i < this.inputPos.length; i++) {
            rect(this.x+this.inputPos[i].x, this.y+this.inputPos[i].y, 12, 10,10);
