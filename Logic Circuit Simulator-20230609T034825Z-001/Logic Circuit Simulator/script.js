...
function restoreGroups(){
    ...
    if(localtypes) {
        ...
        extraKeys.forEach(key => {
            ...
            let parentAccordion = document.querySelector('#flush-collapseCustom');
            let accordionBodyElement = parentAccordion.querySelector('.accordion-body');
            //row
            var rowElement = document.createElement('div');
            rowElement.classList.add('row');
            //button
            let btn = createButton(groupNode.type);
            btn.addClass('btn btn-outline-primary nodebtn');
            btn.elt.setAttribute("onclick", `placeGroup(${createdNodes.length - 1})`);
            //append
            btn.parent(rowElement);
            accordionBodyElement.appendChild(rowElement);
        })
    }
...
function keyPressed() {
    ...
    } else if (keyIsDown(CONTROL) && (key == 'q' || key == 'Q')) {
        if (!isInsideRect(nearest.x, nearest.y, nearest.w, nearest.h, mouseX, mouseY)) return;
        optionNode = nearest;
...
function createBG() {
  ...
  let gradientStart = color('#FFFFFF');
  let gradientEnd = color('#FFFFFF');
  
  // Set up the gradient background
  bg.noFill();
  bg.stroke(0, 50);
  bg.background(gradientStart);
  bg.strokeWeight(2);
  bg.strokeJoin(ROUND);
  let gradient = bg.drawingContext.createLinearGradient(0, 0, width, height);
